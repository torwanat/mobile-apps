...
class Gallery extends Component {
    ...
    selectPhoto = (uri, id, width, height) => {
        this.props.navigation.navigate("s3", { uri: uri, id: id, width: width, height: height, refreshGallery: this.refreshGallery });
    }
            ...
            return (
                <View style={{ flex: 1, backgroundColor: "#F44336" }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-around', padding: 15 }}>
                        <UniversalButton text={"Change layout"} onClick={this.changeLayout} bgColor={"#FF9800"} color={"#FFFFFF"} />
                        <UniversalButton text={"Camera"} onClick={this.goToCamera} bgColor={"#FF9800"} color={"#FFFFFF"} />
                        <UniversalButton text={"Delete selected"} onClick={this.deletePhotos} bgColor={"#FF9800"} color={"#FFFFFF"} />
                    </View>
                    {this.state.photos ? <FlatList data={this.state.photos} renderItem={({ item }) => <FotoItem width={this.state.width} height={this.state.height} imageWidth={item.width} imageHeight={item.height} uri={item.uri} id={item.id} marked={item.marked} onClick={this.selectPhoto} onHold={this.markPhoto} />} keyExtractor={item => item.id} numColumns={this.state.columns} key={this.state.columns} /> : <ActivityIndicator size={"large"} color={"#000000"} />}
                </View>
            );
        } else {
            return (
