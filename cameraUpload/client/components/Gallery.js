...
class Gallery extends Component {
    ...
    componentDidMount = async () => {
        ...
        if (status === "granted") {
            ...
            this.setState({ width: Math.floor((Dimensions.get("window").width - 30) / 5), height: Math.floor((Dimensions.get("window").width - 30) / 5), photos: photos, server: "http://" + ip + ":" + port + "/upload" })
        } else {
    ...
    uploadSelected = async () => {
        ...
        const toSend = this.state.photos.filter(e => {
            return e.marked;
        });

        toSend.forEach((e, i) => {
            const fileName = e.uri.substring(e.uri.lastIndexOf("/") + 1, e.uri.length);
            data.append("" + i, {
                uri: e.uri,
                type: "image/jpeg",
                name: fileName
        ...
        if (json === "OK") {
            ToastAndroid.showWithGravity("Przesłano zdjęcia!", ToastAndroid.SHORT, ToastAndroid.CENTER);
        } else {
            ToastAndroid.showWithGravity("Błąd w przesyłaniu zdjęć", ToastAndroid.SHORT, ToastAndroid.CENTER);
        }
    }
        ...
        if (hasLibraryPermission === null) {
            ...
            return (
                <View style={{ flex: 1, backgroundColor: "#F44336" }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-around', padding: 15, flexWrap: "wrap" }}>
                        ...
                        <UniversalButton text={"Upload selected"} onClick={this.uploadSelected} bgColor={"#FF5722"} color={"#FFFFFF"} />
                        <UniversalButton text={"Server settings"} onClick={this.goToSettings} bgColor={"#FF5722"} color={"#FFFFFF"} />
                    </View>
